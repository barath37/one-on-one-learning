...
import { A as hasKatex, B as sanitizeText3, M as parseGenericTypes, T as getEffectiveHtmlLabels, b as getConfig, g as evaluate, x as getConfig2, z as sanitizeText } from "./chunk-WYO6CB5R-J6ZhahPa.js";
import { a as decodeEntities, r as calculateTextWidth, u as handleUndefinedAttr } from "./chunk-ICXQ74PX-DIjkgJ-o.js";
import { a as userNodeOverrides, i as styles2String, r as solidStateFill, t as compileStyles } from "./chunk-C7G6YPKG-yZpxWm7O.js";
import { n as getSubGraphTitleMargins, t as configureLabelImages } from "./chunk-OGEWGWER-bKK6K7Am.js";
import { t as getIconSVG } from "./chunk-HOUHSVGY-CrRk46YP.js";
import { n as createText } from "./chunk-Q4XR5HBZ-fEFemAd0.js";
import { t as at } from "./rough.esm-BX5QqLti.js";
//#region node_modules/mermaid/dist/chunks/mermaid.core/chunk-ZGVPDNZ5.mjs
...
function generateCirclePoints(centerX, centerY, radius, numPoints, startAngle, endAngle) {
	...
	const angleStep = (endAngle * Math.PI / 180 - startAngleRad) / (numPoints - 1);
...
function mergePaths(roughElement) {
	...
	const getAttr = /* @__PURE__ */ __name((element, attr) => {
		return element?.getAttribute(attr) ?? void 0;
	}, "getAttr");
	if (fillPath) {
		const fillAttrs = {
		...
		Object.entries(fillAttrs).forEach(([attr, value]) => {
			if (value) mergedPath.setAttribute(attr, value);
		});
	}
	if (strokePath) {
		const strokeAttrs = {
...
var intersect_rect_default = /* @__PURE__ */ __name((node, point) => {
	var x = node.x;
	var y = node.y;
	var dx = point.x - x;
	var dy = point.y - y;
...
var createLabel_default = /* @__PURE__ */ __name(async (element, _vertexText, style, isTitle = false, isNode = false) => {
...
var swimlane = /* @__PURE__ */ __name(async (parent, node) => {
	...
	if (node.width <= bbox.width + padding) node.diff = (width - node.width) / 2 - padding;
	else node.diff = -padding;
	...
	if (isLR) {
		...
		labelEl.attr("transform", `translate(${labelCenterX}, ${labelCenterY}) rotate(-90) translate(${-bbox.width / 2}, ${-bbox.height / 2})`);
	} else {
...
var rect = /* @__PURE__ */ __name(async (parent, node) => {
	...
	let text2;
	if (node.labelType === "markdown") text2 = await createText(labelEl, node.label, {
		style: node.labelStyle,
		useHtmlLabels,
		isNode: true,
		width: node.width
	});
	else text2 = await createLabel_default(labelEl, node.label, node.labelStyle || "", false, true);
	let bbox = text2.getBBox();
...
var shapes = {
	rect,
	squareRect: rect,
	roundedWithTitle: /* @__PURE__ */ __name(async (parent, node) => {
		const siteConfig = getConfig2();
		const { themeVariables, handDrawnSeed } = siteConfig;
		...
		if (node.look === "handDrawn") {
			...
			rect2.attr("class", "outer").attr("x", x).attr("y", y).attr("width", width).attr("height", height).attr("data-look", node.look);
	...
	noteGroup: /* @__PURE__ */ __name((parent, node) => {
	...
	divider: /* @__PURE__ */ __name((parent, node) => {
		...
		let rect2;
		if (node.look === "handDrawn") {
			const roughOuterNode = at.svg(shapeSvg).rectangle(x, y, width, height, {
				fill: "lightgrey",
				roughness: .5,
				strokeLineDash: [5],
				stroke: nodeBorder,
			...
			if (node.look === "neo") outerRectClass = "divider";
	...
	kanbanSection: /* @__PURE__ */ __name(async (parent, node) => {
		log.info("Creating subgraph rect for ", node.id, node);
		const siteConfig = getConfig2();
		const { themeVariables, handDrawnSeed } = siteConfig;
		const { clusterBkg, clusterBorder } = themeVariables;
...
var insertCluster = /* @__PURE__ */ __name(async (elem, node) => {
	const cluster = await shapes[node.shape || "rect"](elem, node);
...
function intersectLine(p1, p2, q1, q2) {
	{
		...
		if (Math.abs(r1) < epsilon && Math.abs(r2) < epsilon && sameSign(r1, r2)) return;
...
function generateArcPoints(x1, y1, x2, y2, rx, ry, clockwise) {
	...
	let angleRange = Math.atan2((y2 - centerY) / ry, (x2 - centerX) / rx) - startAngle;
...
async function bowTieRect(parent, node) {
	...
	const calcEllipseRadius = /* @__PURE__ */ __name((totalHeight2) => {
		...
		return [ry2 / (2.5 + totalHeight2 / 50), ry2];
	}, "calcEllipseRadius");
	...
	const w = (node?.width ? node?.width : bbox.width) + labelPaddingX * 2 + sagitta - sagitta;
	const h = totalHeight;
	const { cssStyles } = node;
	const points = [
		{
...
async function card(parent, node) {
	...
	} else polygon = insertPolygonShape(shapeSvg, w, h, points);
...
async function curlyBraceLeft(parent, node) {
	...
	const rectPoints = [
		...
		...generateCirclePoints2(w / 2 + w * .1, -radius, radius, 20, -180, -270),
		...generateCirclePoints2(w / 2 + w * .1, radius, radius, 20, -90, -180),
		{
			x: -w / 2 - radius,
			y: h / 2
		},
	...
	const newCurlyBracePath = createPathFromPoints(points).replace("Z", "");
	...
	node.intersect = function(point) {
		return intersect_default.polygon(node, rectPoints, point);
	};
	return shapeSvg;
}
...
async function curlyBraces(parent, node) {
	...
	const newLeftCurlyBracePath = createPathFromPoints(leftCurlyBracePoints).replace("Z", "");
	const leftCurlyBraceNode = rc.path(newLeftCurlyBracePath, options);
	const newRightCurlyBracePath = createPathFromPoints(rightCurlyBracePoints).replace("Z", "");
...
async function cylinder(parent, node) {
	...
	if (node.width || node.height) {
		...
		const ry2 = originalWidth / 2 / (2.5 + originalWidth / 50);
...
async function datastore(parent, node) {
	...
	const rect2 = await drawRect(parent, node, {
		rx: 0,
		ry: 0,
		classes: cssClasses ?? "",
	...
	if (node.look === "handDrawn") {
		...
		if (!borderPath) return rect2;
		let bbox = null;
		if (borderPath instanceof SVGGraphicsElement) bbox = borderPath.getBBox();
		else return rect2;
	...
	if (datastoreWidth > 0 && datastoreHeight > 0) selection.attr("stroke-dasharray", `${datastoreWidth} ${datastoreHeight}`);
	return rect2;
}
...
async function flippedTriangle(parent, node) {
	...
	if (node.width || node.height) {
		...
		if (node.height < MIN_HEIGHT2) node.height = MIN_HEIGHT2;
		node.width = (node?.width ?? 0) - labelPaddingX - labelPaddingX / 2;
		if (node.width < MIN_WIDTH2) node.width = MIN_WIDTH2;
	}
	const { shapeSvg, bbox, label } = await labelHelper(parent, node, getNodeClasses(node));
...
async function halfRoundedRectangle(parent, node) {
	...
	if (node.width || node.height) {
		...
		if (node.width < minWidth) node.width = minWidth;
	}
	const { shapeSvg, bbox } = await labelHelper(parent, node, getNodeClasses(node));
...
async function iconCircle(parent, node, { config: { themeVariables, flowchart } }) {
	...
	options.stroke = stylesMap.get("fill") ?? mainBkg;
	const iconElem = shapeSvg.append("g");
...
async function iconSquare(parent, node, { config: { themeVariables, flowchart } }) {
	...
	options.stroke = stylesMap.get("fill") ?? mainBkg;
	const iconNode = rc.path(createRoundedRectPathD(x, y, width, height, .1), options);
	const outerWidth = Math.max(width, bbox.width);
	const outerHeight = height + bbox.height + labelPadding;
	const outerNode = rc.rectangle(-outerWidth / 2, -outerHeight / 2, outerWidth, outerHeight, {
		...options,
...
async function labelRect(parent, node) {
	...
	const rect2 = shapeSvg.insert("rect", ":first-child");
	rect2.attr("width", .1).attr("height", .1);
...
async function linedCylinder(parent, node) {
	...
	if (node.width || node.height) {
		...
		if (node.width < MIN_WIDTH3) node.width = MIN_WIDTH3;
		const ry2 = originalWidth / 2 / (2.5 + originalWidth / 50);
		node.height = (node.height ?? 0) - labelPaddingY - ry2 * 3;
		if (node.height < MIN_HEIGHT3) node.height = MIN_HEIGHT3;
	}
	const { shapeSvg, bbox, label } = await labelHelper(parent, node, getNodeClasses(node));
	...
	if (node.look === "handDrawn") {
		...
		shapeSvg.insert(() => innerLine, ":first-child").attr("class", "line");
		cylinder2 = shapeSvg.insert(() => outerNode, ":first-child");
		cylinder2.attr("class", "basic label-container");
		if (cssStyles) cylinder2.attr("style", cssStyles);
	} else {
...
async function linedWaveEdgedRect(parent, node) {
	...
	const labelPaddingY = node.look === "neo" ? 12 : nodePadding;
	if (node.width || node.height) {
		node.width = (node.width ?? 0) * 10 / 11 - labelPaddingX * 2;
		if (node.width < 10) node.width = 10;
		node.height = (node?.height ?? 0) - labelPaddingY * 2;
		if (node.height < 10) node.height = 10;
	}
	const { shapeSvg, bbox, label } = await labelHelper(parent, node, getNodeClasses(node));
	...
	const points = [
		...
		...generateFullSineWavePoints(-w / 2 - w / 2 * .1, finalH / 2, w / 2 + w / 2 * .1, finalH / 2, waveAmplitude, .8),
		{
			x: w / 2 + w / 2 * .1,
			y: -finalH / 2
		},
	...
	label.attr("transform", `translate(${-w / 2 + (node.padding ?? 0) + w / 2 * .1 / 2 - (bbox.x - (bbox.left ?? 0))},${-h / 2 + (node.padding ?? 0) - waveAmplitude / 2 - (bbox.y - (bbox.top ?? 0))})`);
...
async function multiRect(parent, node) {
	...
	node.intersect = function(point) {
		return intersect_default.polygon(node, outerPathPoints, point);
	};
	return shapeSvg;
}
...
async function multiWaveEdgedRectangle(parent, node) {
	...
	const { cssStyles } = node;
	const wavePoints = generateFullSineWavePoints(x - rectOffset2, y + finalH + rectOffset2, x + w - rectOffset2, y + finalH + rectOffset2, waveAmplitude, .8);
	...
	const outerPathPoints = [
		...
		{
			x: x - rectOffset2,
			y: y + finalH + rectOffset2
		},
		...wavePoints,
		{
			x: x + w - rectOffset2,
			y: lastWavePoint.y - rectOffset2
		},
		{
			x: x + w,
			y: lastWavePoint.y - rectOffset2
...
async function note(parent, node, { config: { themeVariables } }) {
	const { labelStyles, nodeStyles } = styles2String(node);
	node.labelStyle = labelStyles;
	if (!(node.useHtmlLabels || getEffectiveHtmlLabels(getConfig()))) node.centerLabel = true;
	const { shapeSvg, bbox, label } = await labelHelper(parent, node, getNodeClasses(node));
...
async function question(parent, node) {
	...
	node.labelStyle = labelStyles;
	const { shapeSvg, bbox } = await labelHelper(parent, node, getNodeClasses(node));
	const s = bbox.width + (node.padding ?? 0) + (bbox.height + (node.padding ?? 0));
	const adjustment = .5;
	const points = [
		{
			x: s / 2,
...
function stateEnd(parent, node, { config: { themeVariables } }) {
	...
	if (node.width || node.height) {
		if ((node.width ?? 0) < 14) node.width = 14;
		if ((node.height ?? 0) < 14) node.height = 14;
	}
	...
	if (node.look !== "handDrawn") circle2.attr("class", "outer-path");
...
function stateStart(parent, node, { config: { themeVariables } }) {
	...
	let circle2;
	if (node.look === "handDrawn") {
		const roughNode = at.svg(shapeSvg).circle(0, 0, node.width, solidStateFill(lineColor));
...
async function taggedRect(parent, node) {
	...
	const w = (node?.width ? node?.width : bbox.width) + labelPaddingX * 2 + tagWidth - tagWidth;
	const h = totalHeight;
	const x = -w / 2;
	const y = -h / 2;
	const { cssStyles } = node;
...
async function taggedWaveEdgedRectangle(parent, node) {
	...
	const tagPoints = [
		...
		...generateFullSineWavePoints(x + w, (y + h) * 1.25, x + w - tagWidth, (y + h) * 1.3, -h * .02, .5)
	];
...
async function tiltedCylinder(parent, node) {
	...
	if (node.width || node.height) {
		...
		if (node.height < MIN_HEIGHT4) node.height = MIN_HEIGHT4;
		const rx2 = originalHeight / 2 / (2.5 + originalHeight / 50);
		node.width = (node.width ?? 0) - labelPadding - rx2 * 3;
		if (node.width < MIN_WIDTH4) node.width = MIN_WIDTH4;
	}
	const { shapeSvg, bbox, label } = await labelHelper(parent, node, getNodeClasses(node));
	...
	node.intersect = function(point) {
		...
		if (ry != 0 && (Math.abs(y) < (node.height ?? 0) / 2 || Math.abs(y) == (node.height ?? 0) / 2 && Math.abs(pos.x - (node.x ?? 0)) > (node.width ?? 0) / 2 - rx)) {
			...
			if (x != 0) x = Math.sqrt(Math.abs(x));
			x = rx - x;
			if (point.x - (node.x ?? 0) > 0) x = -x;
			pos.x += x;
		}
		return pos;
	};
...
async function triangle(parent, node) {
	...
	if (node.width || node.height) {
		node.width = ((node?.width ?? 0) - labelPaddingX) / 2;
		if (node.width < MIN_WIDTH5) node.width = MIN_WIDTH5;
		node.height = node?.height ?? 0;
		if (node.height < MIN_HEIGHT5) node.height = MIN_HEIGHT5;
	}
	const { shapeSvg, bbox, label } = await labelHelper(parent, node, getNodeClasses(node));
...
async function waveEdgedRectangle(parent, node) {
	...
	const points = [
		...
		...generateFullSineWavePoints(-w / 2 - extraW, finalH / 2, w / 2 + extraW, finalH / 2, waveAmplitude, .8),
		{
			x: w / 2 + extraW,
			y: -finalH / 2
		},
...
async function waveRectangle(parent, node) {
	...
	if (node.width || node.height) {
		...
		if (node.width < 20) node.width = 20;
		node.height = node?.height ?? 0;
		if (node.height < 10) node.height = 10;
		const waveAmplitude2 = Math.min(node.height * .2, node.height / 4);
...
var REDUX_THEMES = /* @__PURE__ */ new Set([
	"redux",
	"redux-dark",
	"redux-color",
	"redux-dark-color"
...
async function erBox(parent, node) {
	...
	shapeSvg.selectAll("g:not(:first-child)").each((_, i, nodes) => {
		...
		let translateY = 0;
		if (transform) {
			const translate = RegExp(/translate\(([^,]+),([^)]+)\)/).exec(transform);
			if (translate) {
	...
	for (const [i, row] of rows.entries()) {
		const isEven = (i + 1) % 2 === 0 && row.yOffset !== 0;
...
async function textHelper(parent, node, config, useHtmlLabels, GAP = config.class.padding ?? 12) {
	...
	if (node.annotations.length > 0) {
		...
		annotationGroupHeight = annotationGroup.node().getBBox().height;
	}
...
async function addText2(parentGroup, node, yOffset, styles = []) {
	...
	if (!useHtmlLabels) {
		...
		if (text2.textContent === "" || text2.textContent.includes("&gt")) {
			...
			if (textContent[1] === " ") textChild.textContent = textChild.textContent[0] + " " + textChild.textContent.substring(1);
		}
...
async function classBox(parent, node) {
	...
	if (classNode.members.length === 0 && classNode.methods.length === 0) h += GAP;
	else if (classNode.members.length > 0 && classNode.methods.length === 0) h += GAP * 2;
	const x = -w / 2;
	const y = -h / 2;
	let extraHeight = renderExtraBox ? PADDING * 2 : classNode.members.length === 0 && classNode.methods.length === 0 ? -PADDING : 0;
	if (nodeHeightGreater) extraHeight = PADDING * 2;
	...
	if (classNode.members.length > 0 || classNode.methods.length > 0 || renderExtraBox) {
		...
		const roughLine = rc.line(rectBBox.x, firstLineY, rectBBox.x + rectBBox.width, firstLineY + .001, options);
		shapeSvg.insert(() => roughLine).attr("class", `divider${node.look === "neo" && !useGradient ? " neo-line" : ""}`).attr("style", styles);
	}
	if (renderExtraBox || classNode.members.length > 0 || classNode.methods.length > 0) {
		const secondLineY = annotationGroupHeight + labelGroupHeight + membersGroupHeight + y + GAP * 2 + PADDING;
		const roughLine = rc.line(rectBBox.x, nodeHeightGreater ? Math.max(methodsAreaPlacement, secondLineY) : secondLineY, rectBBox.x + rectBBox.width, (nodeHeightGreater ? Math.max(methodsAreaPlacement, secondLineY) : secondLineY) + .001, options);
		shapeSvg.insert(() => roughLine).attr("class", `divider${node.look === "neo" && !useGradient ? " neo-line" : ""}`).attr("style", styles);
	}
...
async function requirementBox(parent, node) {
	...
	shapeSvg.selectAll(".label").each((_, i, nodes) => {
		...
		if (i === 0 || i === 1) newTranslateX = translateX;
	...
	if (accumulativeHeight > typeHeight + nameHeight + gap) {
		...
		} else roughLine = rc.line(x, lineY, x + totalWidth, lineY, options);
		shapeSvg.insert(() => roughLine).attr("class", "divider");
	}
	updateNodeBounds(node, rect2);
	node.intersect = function(point) {
	...
	if (nodeStyles && node.look !== "handDrawn" && (requirementEdgeLabelBackground || borderColorArray?.length)) shapeSvg.selectAll("path").attr("style", nodeStyles);
	return shapeSvg;
}
...
async function addText3(parentGroup, inputText, yOffset, style = "") {
	...
	if (!useHtmlLabels) {
		const textChild = text2.children[0];
		for (const child of textChild.children) if (style) child.setAttribute("style", style);
		bbox = text2.getBBox();
		bbox.height += 6;
	} else {
		const div = text2.children[0];
...
async function bang(parent, node) {
	...
	let bangElem;
	const path = `M0 0 
    a${r},${r} 1 0,0 ${effectiveWidth * .25},${-1 * effectiveHeight * .1}
    a${r},${r} 1 0,0 ${effectiveWidth * .25},0
    a${r},${r} 1 0,0 ${effectiveWidth * .25},0
    a${r},${r} 1 0,0 ${effectiveWidth * .25},${effectiveHeight * .1}

    a${r},${r} 1 0,0 ${effectiveWidth * .15},${effectiveHeight * .33}
    a${r * .8},${r * .8} 1 0,0 0,${effectiveHeight * .34}
    a${r},${r} 1 0,0 ${-1 * effectiveWidth * .15},${effectiveHeight * .33}

    a${r},${r} 1 0,0 ${-1 * effectiveWidth * .25},${effectiveHeight * .15}
    ...
    a${r * .8},${r * .8} 1 0,0 0,${-1 * effectiveHeight * .34}
  ...
  H0 V0 Z`;
	...
	} else bangElem = shapeSvg.insert("path", ":first-child").attr("class", "basic label-container").attr("style", nodeStyles).attr("d", path);
...
async function cloud(parent, node) {
	...
	let cloudElem;
	const path = `M0 0 
    a${r1},${r1} 0 0,1 ${w * .25},${-1 * w * .1}
    a${r3},${r3} 1 0,1 ${w * .4},${-1 * w * .1}
    a${r2},${r2} 1 0,1 ${w * .35},${w * .2}

    a${r1},${r1} 1 0,1 ${w * .15},${h * .35}
    a${r4},${r4} 1 0,1 ${-1 * w * .15},${h * .65}
  ...
  H0 V0 Z`;
	...
	} else cloudElem = shapeSvg.insert("path", ":first-child").attr("class", "basic label-container").attr("style", nodeStyles).attr("d", path);
	label.attr("transform", `translate(${-bbox.width / 2}, ${-bbox.height / 2})`);
...
async function defaultMindmapNode(parent, node) {
  ...
  ` : `
    ...
    Z
  `;
	if (!node.domId) throw new Error(`defaultMindmapNode: node "${node.id}" is missing a domId \u2014 was render.ts domId prefixing skipped?`);
...
async function mindmapCircle(parent, node) {
	return circle(parent, node, { padding: node.padding ?? 0 });
}
...
//#endregion
export { insertNode as a, positionNode as c, insertCluster as i, setNodeElem as l, clear2 as n, isValidShape as o, createLabel_default as r, labelHelper as s, clear as t, updateNodeBounds as u };
