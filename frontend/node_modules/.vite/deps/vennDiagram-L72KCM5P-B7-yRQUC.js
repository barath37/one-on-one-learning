...
import { R as select_default } from "./src-FJkK6OsU.js";
import { H as setAccDescription, K as setDiagramTitle, U as setAccTitle, a as clear, b as getConfig, c as configureSvgSize, et as darken, f as defaultConfig_default, nt as adjustChannel, rt as isDark, tt as lighten, v as getAccDescription, w as getDiagramTitle, y as getAccTitle } from "./chunk-WYO6CB5R-J6ZhahPa.js";
import { i as cleanAndMerge } from "./chunk-ICXQ74PX-DIjkgJ-o.js";
import { t as selectSvgElement } from "./chunk-VAUOI2AC-CG4QmX_I.js";
import { t as at } from "./rough.esm-BX5QqLti.js";
...
function intersectionArea(circles, stats) {
	...
	if (innerPoints.length > 1) {
		...
		for (let i = 0; i < innerPoints.length; ++i) {
			...
			let arc = null;
			for (let j = 0; j < p1.parentIndex.length; ++j) if (p2.parentIndex.includes(p1.parentIndex[j])) {
				...
				if (width > circle.radius * 2) width = circle.radius * 2;
				if (arc == null || arc.width > width) arc = {
					circle,
					width,
					p1,
					p2,
		...
		for (let i = 1; i < circles.length; ++i) if (circles[i].radius < smallest.radius) smallest = circles[i];
		let disjoint = false;
		for (let i = 0; i < circles.length; ++i) if (distance(circles[i], smallest) > Math.abs(smallest.radius - circles[i].radius)) {
			disjoint = true;
			break;
		}
...
function nelderMead(f, x0, parameters) {
	...
	const zeroDelta = parameters.zeroDelta || .001;
	...
	const psi = parameters.psi !== void 0 ? parameters.psi : -.5;
	const sigma = parameters.sigma !== void 0 ? parameters.sigma : .5;
	let maxDiff;
	const N = x0.length;
...
function wolfeLineSearch(f, pk, current, next, a, c1, c2) {
	...
	function zoom(a_lo, a_high, phi_lo) {
		for (let iteration = 0; iteration < 16; ++iteration) {
			...
			if (phi > phi0 + c1 * a * phiPrime0 || phi >= phi_lo) a_high = a;
			else {
	...
	for (let iteration = 0; iteration < 10; ++iteration) {
		...
		phi = next.fx = f(next.x, next.fxprime);
		phiPrime = dot(next.fxprime, pk);
		if (phi > phi0 + c1 * a * phiPrime0 || iteration && phi >= phi_old) return zoom(a0, a, phi_old);
		if (Math.abs(phiPrime) <= -c2 * phiPrime0) return a;
...
function conjugateGradient(f, initial, params) {
	...
	for (let i = 0; i < maxIterations; ++i) {
		...
		if (!a) scale(pk, current.fxprime, -1);
		else {
...
function venn(sets, parameters = {}) {
	...
	const positions = nelderMead((values) => {
		const current = {};
...
function distanceFromIntersectArea(r1, r2, overlap) {
	if (Math.min(r1, r2) * Math.min(r1, r2) * Math.PI <= overlap + SMALL) return Math.abs(r1 - r2);
...
function addMissingAreas(areas, parameters = {}) {
	...
	if (distinct) {
		...
		for (const area of r) for (let i = 0; i < area.sets.length; i++) {
		...
		for (const area of r) if (area.sets.length < 3) area.size = count.get(toKey(area.sets));
	}
	const ids = [];
	/** @type {Set<string>} */
	const pairs = /* @__PURE__ */ new Set();
	for (const area of r) if (area.sets.length === 1) ids.push(area.sets[0]);
...
function getDistanceMatrices(areas, sets, setids) {
	...
	*/
	...
	areas.filter((x) => x.sets.length === 2).forEach((current) => {
		...
		const distance = distanceFromIntersectArea(Math.sqrt(sets[left].size / Math.PI), Math.sqrt(sets[right].size / Math.PI), current.size);
		distances[left][right] = distances[right][left] = distance;
		let c = 0;
		if (current.size + 1e-10 >= Math.min(sets[left].size, sets[right].size)) c = 1;
...
function constrainedMDSGradient(x, fxprime, distances, constraints) {
	for (let i = 0; i < fxprime.length; ++i) fxprime[i] = 0;
	let loss = 0;
	for (let i = 0; i < distances.length; ++i) {
		...
		for (let j = i + 1; j < distances.length; ++j) {
			...
			if (constraint > 0 && distance <= dij || constraint < 0 && distance >= dij) continue;
			loss += 2 * delta * delta;
...
function constrainedMDSLayout(areas, params = {}) {
	...
	const sets = [];
	const setids = {};
	for (const area of areas) if (area.sets.length === 1) {
	...
	for (let i = 0; i < restarts; ++i) {
		const current = conjugateGradient(obj, zeros(distances.length * 2).map(Math.random), params);
	...
	if (params.history) for (const h of params.history) scale(h.x, norm);
	return circles;
}
/**
...
function greedyLayout(areas, params) {
	...
	for (const current of areas) {
		...
		if (current.size + SMALL >= Math.min(circles[left].size, circles[right].size)) weight = 0;
	...
	*/
	...
	for (let i = 1; i < mostOverlapped.length; ++i) {
		...
		if (overlap.length === 0) throw "ERROR: missing pairwise overlap information";
...
function lossFunction(circles, overlaps) {
	...
	for (const area of overlaps) {
		if (area.sets.length === 1) continue;
		/** @type {number} */
		let overlap;
		...
		} else overlap = intersectionArea(area.sets.map((d) => circles[d]));
...
function orientateCircles(circles, orientation, orientationOrder) {
	if (orientationOrder == null) circles.sort((a, b) => b.radius - a.radius);
	else circles.sort(orientationOrder);
	if (circles.length > 0) {
	...
	if (circles.length === 2) {
		if (distance(circles[0], circles[1]) < Math.abs(circles[1].radius - circles[0].radius)) {
...
function disjointCluster(circles) {
	...
	function find(circle) {
		if (circle.parent !== circle) circle.parent = find(circle.parent);
	...
	for (let i = 0; i < circles.length; ++i) for (let j = i + 1; j < circles.length; ++j) {
		...
		if (distance(circles[i], circles[j]) + 1e-10 < maxDistance) union(circles[j], circles[i]);
	}
	...
	for (let i = 0; i < circles.length; ++i) {
		...
		if (!disjointClusters.has(setid)) disjointClusters.set(setid, []);
...
function getBoundingBox(circles) {
	const minMax = (d) => {
		return {
			max: circles.reduce((acc, c) => Math.max(acc, c[d] + c.radius), Number.NEGATIVE_INFINITY),
			min: circles.reduce((acc, c) => Math.min(acc, c[d] - c.radius), Number.POSITIVE_INFINITY)
		};
	};
	return {
		xRange: minMax("x"),
...
function normalizeSolution(solution, orientation, orientationOrder) {
	if (orientation == null) orientation = Math.PI / 2;
	...
	*/
	function addCluster(cluster, right, bottom) {
		...
		/** @type {number} */
		let yOffset;
		if (right) xOffset = returnBounds.xRange.max - bounds.xRange.min + spacing;
		else {
			xOffset = returnBounds.xRange.max - bounds.xRange.max;
			const centreing = (bounds.xRange.max - bounds.xRange.min) / 2 - (returnBounds.xRange.max - returnBounds.xRange.min) / 2;
			if (centreing < 0) xOffset += centreing;
		}
		if (bottom) yOffset = returnBounds.yRange.max - bounds.yRange.min + spacing;
		else {
...
function toObjectNotation(circles) {
	...
	const r = {};
	for (const circle of circles) r[circle.setid] = circle;
	return r;
}
/**
* @param {{[setid: string]: {x: number, y: number, radius: number}}} solution
...
function fromObjectNotation(solution) {
	return Object.keys(solution).map((id) => Object.assign(solution[id], { setid: id }));
}
/**
...
function VennDiagram(options = {}) {
	let useViewBox = false, width = 600, height = 350, padding = 15, duration = 1e3, orientation = Math.PI / 2, normalize = true, scaleToFit = null, wrap = true, styled = true, fontSize = null, orientationOrder = null, distinct = false, round = null, symmetricalTextCentre = options && options.symmetricalTextCentre ? options.symmetricalTextCentre : false, colourMap = {}, colourScheme = options && options.colourScheme ? options.colourScheme : options && options.colorScheme ? options.colorScheme : [
		...
		if (key in colourMap) return colourMap[key];
		var ret = colourMap[key] = colourScheme[colourIndex];
		colourIndex += 1;
		if (colourIndex >= colourScheme.length) colourIndex = 0;
		return ret;
	...
	function chart(selection) {
		...
		data.forEach((datum) => {
			if (datum.size == 0 && datum.sets.length == 1) toRemove.add(datum.sets[0]);
		});
		...
		if (data.length > 0) {
			...
			if (normalize) solution = normalizeSolution(solution, orientation, orientationOrder);
		...
		const labels = {};
		data.forEach((datum) => {
			if (datum.label) labels[datum.sets] = datum.label;
		});
		function label(d) {
			if (d.sets in labels) return labels[d.sets];
			if (d.sets.length == 1) return "" + d.sets[0];
		}
		selection.selectAll("svg").data([circles]).enter().append("svg");
		const svg = selection.select("svg");
		if (useViewBox) svg.attr("viewBox", `0 0 ${width} ${height}`);
		...
		function pathTween(d) {
			return (t) => {
				return intersectionAreaPath(d.sets.map((set) => {
		...
		if (styled) {
			...
			enterText.style("fill", (d) => {
				if (d.colour || d.color) return "#FFF";
				if (options.textFill) return options.textFill;
				return d.sets.length == 1 ? colours(d.sets) : "#444";
			});
		}
		function asTransition(s) {
			if (typeof s.transition === "function") return s.transition("venn").duration(duration);
			return s;
		}
...
function wrapText(circles, labeller) {
	return function(data) {
		...
		const minChars = (label.length + words.length) / 3;
		let word = words.pop();
		let line = [word];
		let lineNumber = 0;
		...
		const initial = .35 - lineNumber * lineHeight / 2;
...
function computeTextCentre(interior, exterior, symmetricalTextCentre) {
	...
	let valid = true;
	for (const i of interior) if (distance(ret, i) > i.radius) {
		valid = false;
		break;
	}
	for (const e of exterior) if (distance(ret, e) < e.radius) {
		valid = false;
		break;
	}
	if (valid) return ret;
	if (interior.length == 1) return {
		x: interior[0].x,
		y: interior[0].y
	};
	const areaStats = {};
	intersectionArea(interior, areaStats);
	if (areaStats.arcs.length === 0) return {
		x: 0,
		y: -1e3,
		disjoint: true
	};
	if (areaStats.arcs.length == 1) return {
		x: areaStats.arcs[0].circle.x,
		y: areaStats.arcs[0].circle.y
	};
	if (exterior.length) return computeTextCentre(interior, []);
	return getCenter(areaStats.arcs.map((a) => a.p1));
}
function getOverlappingCircles(circles) {
	const ret = {};
	const circleids = Object.keys(circles);
	for (const circleid of circleids) ret[circleid] = [];
	for (let i = 0; i < circleids.length; i++) {
		...
		for (let j = i + 1; j < circleids.length; ++j) {
			...
			if (d + b.radius <= a.radius + 1e-10) ret[cj].push(ci);
			else if (d + a.radius <= b.radius + 1e-10) ret[ci].push(cj);
		}
	}
	return ret;
}
function computeTextCentres(circles, areas, symmetricalTextCentre) {
	...
	for (let i = 0; i < areas.length; ++i) {
		...
		for (let j = 0; j < area.length; ++j) {
			...
			for (let k = 0; k < overlaps.length; ++k) exclude[overlaps[k]] = true;
		}
		const interior = [];
		const exterior = [];
		for (let setid in circles) if (setid in areaids) interior.push(circles[setid]);
		...
		ret[area] = centre;
		if (centre.disjoint && areas[i].size > 0) console.log("WARNING: area " + area + " not represented on screen");
	}
	return ret;
}
/**
...
function layout(data, options = {}) {
	const { lossFunction: loss, layoutFunction: layout = venn, normalize = true, orientation = Math.PI / 2, orientationOrder, width = 600, height = 350, padding = 15, scaleToFit = false, symmetricalTextCentre = false, distinct, round = 2 } = options;
	...
	function genDistinctPath(sets) {
		let r = "";
		for (const e of helpers) if (e.has.size > sets.length && sets.every((s) => e.has.has(s))) r += " " + e.path;
		return r;
	}
...
//#endregion
//#region node_modules/mermaid/dist/chunks/mermaid.core/vennDiagram-L72KCM5P.mjs
var parser = (function() {
	var o = /* @__PURE__ */ __name(function(k, v, o2, l) {
		...
		24,
		27
	], $V8 = [1, 57], $V9 = [1, 56], $Va = [1, 58], $Vb = [1, 59], $Vc = [1, 60], $Vd = [
		7,
		8,
		11,
		12,
		...
		performAction: /* @__PURE__ */ __name(function anonymous(yytext, yyleng, yylineno, yy, yystate, $$, _$) {
			...
			switch (yystate) {
				...
				case 9:
					...
					if (yy.setIndentMode) yy.setIndentMode(true);
					break;
				case 10:
				...
				case 12:
					...
					break;
				case 13:
					if ($$[$0].length < 2) throw new Error("union requires multiple identifiers");
					if (yy.validateUnionIdentifiers) yy.validateUnionIdentifiers($$[$0]);
					yy.addSubsetData($$[$0], void 0, void 0);
					if (yy.setIndentMode) yy.setIndentMode(true);
					break;
				case 14:
					if ($$[$0 - 1].length < 2) throw new Error("union requires multiple identifiers");
					if (yy.validateUnionIdentifiers) yy.validateUnionIdentifiers($$[$0 - 1]);
					yy.addSubsetData($$[$0 - 1], $$[$0], void 0);
					if (yy.setIndentMode) yy.setIndentMode(true);
					break;
				case 15:
					if ($$[$0 - 2].length < 2) throw new Error("union requires multiple identifiers");
					if (yy.validateUnionIdentifiers) yy.validateUnionIdentifiers($$[$0 - 2]);
					yy.addSubsetData($$[$0 - 2], void 0, parseFloat($$[$0]));
					if (yy.setIndentMode) yy.setIndentMode(true);
					break;
				case 16:
					if ($$[$0 - 3].length < 2) throw new Error("union requires multiple identifiers");
					if (yy.validateUnionIdentifiers) yy.validateUnionIdentifiers($$[$0 - 3]);
					yy.addSubsetData($$[$0 - 3], $$[$0 - 2], parseFloat($$[$0]));
					if (yy.setIndentMode) yy.setIndentMode(true);
					break;
				case 17:
		...
		return {
			...
			performAction: /* @__PURE__ */ __name(function anonymous(yy, yy_, $avoiding_name_collisions, YY_START) {
				switch ($avoiding_name_collisions) {
					...
					case 4: break;
					case 5:
						if (yy.setIndentMode) yy.setIndentMode(false);
						this.begin("INITIAL");
					...
					case 13: return 17;
					case 14:
						if (yy.consumeIndentText) yy.consumeIndentText = false;
						else return 19;
						break;
					case 15: return 24;
					case 16:
...
var addStyleData = /* @__PURE__ */ __name((identifierList, data) => {
	...
	const styles = {};
	for (const [key, value] of data) styles[key] = normalizeStyleValue(value) ?? value;
	styleEntries.push({
		targets,
		styles
	});
...
var validateUnionIdentifiers = /* @__PURE__ */ __name((identifierList) => {
	const unknown = normalizeIdentifierList(identifierList).filter((identifier) => !knownSets.has(identifier));
	if (unknown.length > 0) throw new Error(`unknown set identifier: ${unknown.join(", ")}`);
...
function buildStyleByKey(styleData) {
	...
	for (const entry of styleData) {
		...
		if (existing) Object.assign(existing, entry.styles);
		else map.set(key, { ...entry.styles });
	}
	return map;
}
...
var draw = /* @__PURE__ */ __name((_text, id, _version, diagObj) => {
	...
	if (title) svg.append("text").text(title).attr("class", "venn-title").attr("font-size", `${32 * scale}px`).attr("text-anchor", "middle").attr("dominant-baseline", "middle").attr("x", "50%").attr("y", 32 * scale).style("fill", themeVariables.vennTitleTextColor || themeVariables.titleColor);
	...
	if (textNodes2.length > 0) renderTextNodes(config, layoutByKey, dummyD3root, textNodes2, scale, styleByKey);
	...
	dummyD3root.selectAll(".venn-circle").each(function(d, i) {
		...
		const setsKey = stableSetsKey([...d.sets].sort());
		...
		const fillOpacity = customStyle?.["fill-opacity"] ?? .1;
		...
		if (isHandDrawn && roughSvg) {
			...
			if (layoutArea && layoutArea.circles.length > 0) {
				...
				const roughNode = roughSvg.circle(c.x, c.y, c.radius * 2, {
					roughness: .7,
					seed: handDrawnSeed,
					fill: transparentize(baseColor, .7),
					fillStyle: "hachure",
					fillWeight: 2,
					hachureGap: 8,
		...
		} else group.select("path").style("fill", baseColor).style("fill-opacity", fillOpacity).style("stroke", strokeColor).style("stroke-width", strokeWidthVal).style("stroke-opacity", .95);
	...
	if (isHandDrawn && roughSvg) dummyD3root.selectAll(".venn-intersection").each(function(d) {
		const group = select_default(this);
		const setsKey = stableSetsKey([...d.sets].sort());
		const customStyle = styleByKey.get(setsKey);
		...
		if (customFill) {
			...
			if (pathD) {
				const roughNode = roughSvg.path(pathD, {
					roughness: .7,
					seed: handDrawnSeed,
					fill: transparentize(customFill, .3),
		...
		} else group.select("path").style("fill-opacity", 0);
		...
		dummyD3root.selectAll(".venn-intersection text").style("font-size", `${48 * scale}px`).style("fill", (e) => {
			const setsKey = stableSetsKey([...e.sets].sort());
	...
	if (dummySvg && "childNodes" in dummySvg) for (const child of [...dummySvg.childNodes]) vennGroup.node()?.appendChild(child);
...
function renderTextNodes(config, layoutByKey, dummyD3root, textNodes2, scale, styleByKey) {
	...
	const textGroup = dummyD3root.select("svg").append("g").attr("class", "venn-text-nodes");
	...
	for (const [key, nodes] of nodesByArea.entries()) {
		...
		if (innerRadius === 0 && Number.isFinite(minCircleRadius)) innerRadius = minCircleRadius * .6;
		const areaGroup = textGroup.append("g").attr("class", "venn-text-area").attr("font-size", `${40 * scale}px`);
		if (useDebugLayout) areaGroup.append("circle").attr("class", "venn-text-debug-circle").attr("cx", centerX).attr("cy", centerY).attr("r", innerRadius).attr("fill", "none").attr("stroke", "purple").attr("stroke-width", 1.5 * scale).attr("stroke-dasharray", `${6 * scale} ${4 * scale}`);
		const innerWidth = Math.max(80 * scale, innerRadius * 2 * .95);
		const innerHeight = Math.max(60 * scale, innerRadius * 2 * .95);
		const labelOffset = (area.data.label && area.data.label.length > 0 ? Math.min(32 * scale, innerRadius * .25) : 0) + (nodes.length <= 2 ? 30 * scale : 0);
		...
		for (const [i, node] of nodes.entries()) {
			...
			const x = startX + cellWidth * (col + .5);
			const y = startY + cellHeight * (row + .5);
			if (useDebugLayout) areaGroup.append("rect").attr("class", "venn-text-debug-cell").attr("x", startX + cellWidth * col).attr("y", startY + cellHeight * row).attr("width", cellWidth).attr("height", cellHeight).attr("fill", "none").attr("stroke", "teal").attr("stroke-width", 1 * scale).attr("stroke-dasharray", `${4 * scale} ${3 * scale}`);
			const boxWidth = cellWidth * .9;
			const boxHeight = cellHeight * .9;
...
function ensurePairwiseSubsets(subsets2) {
	...
	const individualSetSizes = new Map(subsets2.filter((s) => s.sets.length === 1 && s.size !== void 0).map((s) => [s.sets[0], s.size]));
	const synthetic = [];
	for (const subset of subsets2) {
		if (subset.sets.length < 3) continue;
		const members = [...subset.sets].sort();
		for (let i = 0; i < members.length - 1; i++) for (let j = i + 1; j < members.length; j++) {
