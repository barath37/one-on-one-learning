...
import { J as setupGraphViewbox, et as darken, f as defaultConfig_default, rt as isDark, tt as lighten, x as getConfig2, z as sanitizeText } from "./chunk-WYO6CB5R-J6ZhahPa.js";
...
import { a as insertNode, c as positionNode, i as insertCluster } from "./chunk-ZGVPDNZ5-NwURWyhR.js";
import { t as getIconStyles } from "./chunk-5VM5RSS4-D50WUAUX.js";
//#region node_modules/mermaid/dist/chunks/mermaid.core/kanban-definition-HUTT4EX6.mjs
var parser = (function() {
...
var getSection = /* @__PURE__ */ __name((level) => {
	...
	for (let i = nodes.length - 1; i >= 0; i--) {
		if (nodes[i].level === sectionLevel && !lastSection) lastSection = nodes[i];
		if (nodes[i].level < sectionLevel) throw new Error("Items without section detected, found section (\"" + nodes[i].label + "\")");
	}
...
var addNode = /* @__PURE__ */ __name((level, id, descr, type, shapeData) => {
	...
	switch (type) {
		...
		case nodeType.RECT:
		case nodeType.HEXAGON: padding *= 2;
	}
	const node = {
	...
	if (shapeData !== void 0) {
		...
		if (doc?.icon) node.icon = doc?.icon.toString();
		if (doc?.assigned) node.assigned = doc?.assigned.toString();
		if (doc?.ticket) node.ticket = doc?.ticket.toString();
		if (doc?.priority) node.priority = doc?.priority;
	}
	const section = getSection(level);
	if (section) node.parentId = section.id || "kbn" + cnt++;
...
var kanbanDb_default = {
	...
	getType: /* @__PURE__ */ __name((startStr, endStr) => {
		...
		switch (startStr) {
			...
			case "(": return endStr === ")" ? nodeType.ROUNDED_RECT : nodeType.CLOUD;
	...
	setElementForId: /* @__PURE__ */ __name((id, element) => {
		elements[id] = element;
	}, "setElementForId"),
	decorateNode: /* @__PURE__ */ __name((decoration) => {
		...
		if (decoration.class) node.cssClasses = sanitizeText(decoration.class, config);
	}, "decorateNode"),
	type2Str: /* @__PURE__ */ __name((type) => {
		switch (type) {
	...
	getElementById: /* @__PURE__ */ __name((id) => elements[id], "getElementById")
};
var kanbanRenderer_default = { draw: /* @__PURE__ */ __name(async (text, id, _version, diagObj) => {
	log.debug("Rendering kanban diagram\n" + text);
	const data4Layout = diagObj.db.getData();
	const conf = getConfig2();
	conf.htmlLabels = false;
	const svg = selectSvgElement(id);
	for (const node of data4Layout.nodes) node.domId = `${id}-${node.id}`;
	...
	for (const section of sections2) {
		...
		for (const item of sectionItems) {
			...
			const bbox = (await insertNode(nodesElem, item, { config: conf })).node().getBBox();
