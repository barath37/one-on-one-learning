...
import { T as getEffectiveHtmlLabels, b as getConfig, x as getConfig2 } from "./chunk-WYO6CB5R-J6ZhahPa.js";
import { $ as cardinal_default, J as step_default, K as stepAfter, Q as catmullRom_default, X as monotoneX, Y as natural_default, Z as monotoneY, et as basis_default, g as utils_default, nt as bumpY, q as stepBefore, rt as linear_default, tt as bumpX, u as handleUndefinedAttr } from "./chunk-ICXQ74PX-DIjkgJ-o.js";
import { t as line_default } from "./line-B4IS5LAO.js";
import { i as styles2String, n as isLabelStyle } from "./chunk-C7G6YPKG-yZpxWm7O.js";
...
import { r as createLabel_default } from "./chunk-ZGVPDNZ5-NwURWyhR.js";
import { i as markerOffsets2, n as getLineFunctionsWithOffset, r as markerOffsets, t as computeLabelTransform } from "./chunk-7BUUIJ7U-DtPq2Pz8.js";
...
var addEdgeMarker = /* @__PURE__ */ __name((svgPath, position, arrowType, url, id, diagramType, useMargin = false, strokeColor) => {
	...
	const originalMarkerId = `${id}_${diagramType}-${arrowTypeInfo.type}${position === "start" ? "Start" : "End"}${useMargin && marginSupport ? "-margin" : ""}`;
	if (strokeColor && strokeColor.trim() !== "") {
		const coloredMarkerId = `${originalMarkerId}_${strokeColor.replace(/[^\dA-Za-z]/g, "_")}`;
		if (!document.getElementById(coloredMarkerId)) {
			const originalMarker = document.getElementById(originalMarkerId);
			if (originalMarker) {
				const coloredMarker = originalMarker.cloneNode(true);
				coloredMarker.id = coloredMarkerId;
				coloredMarker.querySelectorAll("path, circle, line").forEach((path) => {
					path.setAttribute("stroke", strokeColor);
					if (arrowTypeInfo.fill) path.setAttribute("fill", strokeColor);
				});
				originalMarker.parentNode?.appendChild(coloredMarker);
			}
		}
		svgPath.attr(`marker-${position}`, `url(${url}#${coloredMarkerId})`);
	} else svgPath.attr(`marker-${position}`, `url(${url}#${originalMarkerId})`);
}, "addEdgeMarker");
...
var getLabelStyles = /* @__PURE__ */ __name((styleArray) => {
	...
	if (typeof styleArray === "string") return styleArray;
...
var insertEdgeLabel = /* @__PURE__ */ __name(async (elem, edge) => {
	...
	const labelElement = await createText(elem, edge.label, {
		...
		isNode: false,
		markdown: isMarkdown,
		width: isMarkdown ? void 0 : void 0
	}, config);
...
var orthogonalizeToLabelClippedPoints = /* @__PURE__ */ __name((edge, points) => {
	if (!edge?.isLabelEdge || !edge?.id?.endsWith("-to-label") || !Array.isArray(points)) return points;
	if (points.length !== 2) return points;
	const [start, end] = points;
	const dx = Math.abs(end.x - start.x);
	const dy = Math.abs(end.y - start.y);
	if (dx < .001 || dy < .001) return points;
	if (dy >= dx) return [
		start,
		{
			x: start.x,
...
var cutPathAtIntersect = /* @__PURE__ */ __name((_points, boundaryNode) => {
	...
	_points.forEach((point2) => {
		...
		if (!outsideNode(boundaryNode, point2) && !isInside) {
			...
			else log.warn("abc88 no intersect", inter, points);
			isInside = true;
		} else {
...
var findAdjacentPoint = /* @__PURE__ */ __name(function(pointA, pointB, distance) {
	...
	const ratio = distance / Math.sqrt(xDiff * xDiff + yDiff * yDiff);
	return {
...
var fixCorners = /* @__PURE__ */ __name(function(lineData) {
	...
	const newLineData = [];
	for (let i = 0; i < lineData.length; i++) if (cornerPointPositions.includes(i)) {
		...
		if (Math.abs(nextPoint.x - prevPoint.x) > 10 && Math.abs(nextPoint.y - prevPoint.y) >= 10) {
			...
			const r = 5;
			if (cornerPoint.x === newPrevPoint.x) newCornerPoint = {
		...
		} else log.debug("Corner point skipping fixing", Math.abs(nextPoint.x - prevPoint.x), Math.abs(nextPoint.y - prevPoint.y));
		newLineData.push(newCornerPoint, newNextPoint);
	} else newLineData.push(lineData[i]);
	return newLineData;
}, "fixCorners");
var generateDashArray = /* @__PURE__ */ __name((len, oValueS, oValueE) => {
	...
	const dashLength = 2;
	const gapLength = 2;
	const numberOfPairs = Math.floor(middleLength / 4);
	return `0 ${oValueS} ${Array(numberOfPairs).fill(`${dashLength} ${gapLength}`).join(" ")} ${oValueE}`;
}, "generateDashArray");
var insertEdge = /* @__PURE__ */ __name(function(elem, edge, clusterDb, diagramType, startNode, endNode, diagramId, skipIntersect = false) {
	if (!diagramId) throw new Error(`insertEdge: missing diagramId for edge "${edge.id}" \u2014 edge IDs require a diagram prefix for uniqueness`);
	...
	if (edgeCurveType !== "rounded") lineData = fixCorners(lineData);
	...
	switch (edge.thickness) {
		...
		case "invisible":
			...
			break;
		default: strokeClasses = "edge-thickness-normal";
	}
	switch (edge.pattern) {
		case "solid":
		...
		case "dashed":
			...
			break;
		default: strokeClasses += " edge-pattern-solid";
	}
	let svgPath;
	...
	let animationClass = "";
	if (edge.animate) animationClass = "edge-animation-fast";
	if (edge.animation) animationClass = "edge-animation-" + edge.animation;
	...
	if (edge.look === "handDrawn") {
		...
		if (edge.look === "neo" && !animatedEdge) {
			const mOffset = `stroke-dasharray: ${edge.pattern === "dotted" || edge.pattern === "dashed" ? generateDashArray(len, oValueS, oValueE) : `0 ${oValueS} ${len - oValueS - oValueE} ${oValueE}`}; stroke-dashoffset: 0;`;
	...
	if (edge.showPoints) lineData.forEach((point3) => {
	...
	if (!utils_default.isLabelCoordinateInPath(point2, svgPath.attr("d"))) pointsHasChanged = true;
	let paths = {};
...
function generateRoundedPath(points, radius) {
	...
	for (let i = 0; i < size; i++) {
		...
		if (i === 0) path += `M${currPoint.x},${currPoint.y}`;
		else if (i === size - 1) path += `L${currPoint.x},${currPoint.y}`;
		else {
...
var markers = {
	...
	barbNeo: /* @__PURE__ */ __name((elem, type, id) => {
	...
	only_one: /* @__PURE__ */ __name((elem, type, id) => {
	...
	zero_or_one: /* @__PURE__ */ __name((elem, type, id) => {
	...
	one_or_more: /* @__PURE__ */ __name((elem, type, id) => {
	...
	zero_or_more: /* @__PURE__ */ __name((elem, type, id) => {
	...
	only_one_neo: /* @__PURE__ */ __name((elem, type, id) => {
		const { themeVariables } = getConfig();
	...
	zero_or_one_neo: /* @__PURE__ */ __name((elem, type, id) => {
		const { themeVariables } = getConfig();
	...
	one_or_more_neo: /* @__PURE__ */ __name((elem, type, id) => {
		const { themeVariables } = getConfig();
		const { strokeWidth } = themeVariables;
	...
	zero_or_more_neo: /* @__PURE__ */ __name((elem, type, id) => {
		const { themeVariables } = getConfig();
		const { strokeWidth, mainBkg } = themeVariables;
	...
	requirement_arrow: /* @__PURE__ */ __name((elem, type, id) => {
	...
	requirement_contains: /* @__PURE__ */ __name((elem, type, id) => {
	...
	requirement_arrow_neo: /* @__PURE__ */ __name((elem, type, id) => {
		const { themeVariables } = getConfig();
		const { strokeWidth } = themeVariables;
	...
	requirement_contains_neo: /* @__PURE__ */ __name((elem, type, id) => {
		const { themeVariables } = getConfig();
		const { strokeWidth } = themeVariables;
...
//#endregion
export { markers_default as a, insertEdgeLabel as i, edgeLabels as n, positionEdgeLabel as o, insertEdge as r, terminalLabels as s, clear as t };
